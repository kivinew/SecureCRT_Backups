...
COMMANDS: Dict[str, str] = {
    ...
    "register_info": "display ont register-info {port} {ont}",
}
...
class GPONConfig:
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.scroll_lines: int = kwargs.get("scroll_lines", 32)
...
class GPON:
    ...
    def __init__(self, ont: Optional[Ont] = None, config: Optional[GPONConfig] = None) -> None:
        ...
        self._ont_info_fetched: bool = False
    ...
    def get_ont_info(self) -> None:
        ...
        for field in ("status", "serial", "description", "distance", "uptime", "downtime", "downcause",
                      "memory_usage", "cpu_temp", "cpu_usage"):
            val = self.parse(output, PATTERNS[field])
            if val:
                self.data[field] = val
        self._ont_info_fetched = True
    ...
    def diagnose(self) -> str:
        ...
        self.send(f"scroll {self.config.scroll_lines}", max_more=0)
        self.get_ont_info()
    ...
    def diagnose_optics_only(self) -> str:
        if not self.ont:
            return "ONT не найдена."
        self.send(COMMANDS["interface_gpon"].format(frame=self.ont.frame, slot=self.ont.slot), max_more=0)
        self.get_optics()
        ...
        lines = [
            ...
            f"ONT Rx (дБм): {self.data['ont_rx_power']}",
            f"OLT Rx (дБм): {self.data['olt_rx_power']}",
        ]
        if self.data["upstream_errors"] or self.data["downstream_errors"]:
            lines.append(f"Upstream errors: {self.data['upstream_errors']}")
            lines.append(f"Downstream errors: {self.data['downstream_errors']}")
        if self.data["troubleshooting"]:
            lines.append(f"\nРекомендации:\n{self.data['troubleshooting']}")
        return "\n".join(lines)

    def diagnose_register_only(self) -> str:
        if not self.ont:
            return "ONT не найдена."
        self.send(COMMANDS["interface_gpon"].format(frame=self.ont.frame, slot=self.ont.slot), max_more=0)
        output = self.send(COMMANDS["register_info"].format(port=self.ont.port, ont=self.ont.ont), max_more=0)
        self.send("quit", max_more=0)
        ...
        lines = [
            ...
            f"Дескрипшн = {self.data['description']}",
            f"PON SN = {self.data['serial']}",
            f"Статус регистрации: {status}",
            f"Время с последней регистрации (с): {age}",
        ]
        return "\n".join(lines)
    ...
    def _build_report_online(self) -> str:
        ...
        lines.append(f"Память: {self.data['memory_usage']}%")
        lines.append(f"CPU: {self.data['cpu_usage']}%")
        lines.append(f"Температура CPU: {self.data['cpu_temp']}°C")
...
class GPONDiagnostics:
    """Оркестратор диагностики с поддержкой режимов командной строки."""

    def __init__(self) -> None:
        self.modes = self._parse_arguments()
        self.gpon = GPON()
        self.gpon.config.lan_port_restart = not self.modes["no_actions"]
    ...
    def _parse_arguments(self) -> dict:
        ...
        try:
            if _g_crt.Arguments.Count > 0:
                for i in range(_g_crt.Arguments.Count):
                    arg = _g_crt.Arguments[i]
                    if arg == "-n":
    ...
    def run(self) -> None:
        try:
            ...
            if not mem_buffer:
                _g_crt.Screen.Send("\rdisplay ont info by-desc ")
                return

            _g_crt.Screen.Send("\n")
            last_line = _g_crt.Screen.ReadString("#", 1)
            if "(config)" not in last_line.strip():
                _g_crt.Screen.Send("quit\r")

            ont = self.gpon.detect(mem_buffer)
            if not ont:
                return

            if self.modes["delete"]:
                ont.delete_ont()
                pyperclip.copy(f"ONT {ont} успешно удалена.\n")
                return

            self.gpon.send(f"scroll {self.gpon.config.scroll_lines}", max_more=0)
            self.gpon.get_ont_info()

            if self.modes["register_only"]:
                report = self.gpon.diagnose_register_only()
            elif self.modes["optics_only"]:
                report = self.gpon.diagnose_optics_only()
            else:
                report = self.gpon.diagnose()

            pyperclip.copy(report)

        except Exception as e:
            error_line = traceback.extract_tb(e.__traceback__)[-1].lineno
            _g_crt.Dialog.MessageBox(f"Ошибка в строке № {error_line}:\n{e}")
            _g_crt.Screen.Send("display ont info ")
